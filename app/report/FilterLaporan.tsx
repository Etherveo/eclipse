...
export default function FilterLaporanUser({ transactions, user }: { transactions: any[]; user?: { role?: string } }) {
  ...
  const applyFilter = () => {
    ...
    // UBAH BARIS INI: Hapus /admin, biarkan /report saja
    router.push(`/report?${params.toString()}`);
  };

  const handleExportExcel = () => {
    // Format Excel tanpa kolom "Oleh" karena ini khusus user pribadi
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Laporan Pribadi");
    XLSX.writeFile(wb, `Laporan_Pribadi_${filterType}_${new Date().getTime()}.xlsx`);
  };
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-end print:hidden mb-6">
      <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
        <div>
          ...
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500 text-sm w-full md:w-48 bg-white">
        ...
        {filterType === 'spesifik_hari' && (
          <div>
            ...
            <input type="date" value={specificDate} onChange={(e) => setSpecificDate(e.target.value)} className="border border-gray-300 rounded-lg p-2.5 outline-none focus:ring-2 focus:ring-emerald-500 text-sm" />
          </div>
        )}
      ...
      <div className="flex gap-2 w-full md:w-auto mt-4 md:mt-0">
        ...
        <button 
          ...
          disabled={!isExportAllowed}
          className="flex-1 md:flex-none bg-emerald-600 text-white px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-emerald-700 transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
          📊 Ekspor Excel
        </button>
