...
export default async function AdminPinjamPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-5xl mx-auto mb-6 flex justify-between items-center text-sm text-gray-600">
        <Link href="/admin" className="hover:text-rose-600 hover:underline font-medium">← Kembali ke Dashboard</Link>
        <p>Login sebagai: <span className="font-bold">{user.name}</span></p>
      </div>
      <FormPinjaman backRoute="/admin" role={user.role} />
    </main>
  );
}