...
export default async function AdminDashboard() {
  ...
  // 2. Fetch Data Transaksi (Difilter khusus group_id ini saja)
  // Perhatikan: users (name) diubah jadi users (full_name) menyesuaikan DB V2
  ...
  // 3. Fetch Data Target (Difilter khusus group_id)
  ...
  // 4. Fetch Data QRIS (Difilter khusus group_id)
  ...
  // FETCH DATA ANGGOTA PENDING (Tambahkan blok ini)
  ...
  // FETCH REQUEST PINJAMAN (Khusus untuk Admin)
  const { data: loanRequests } = await supabase
    .from('system_requests')
    .select('*')
    .eq('group_id', user.group_id)
    .eq('type', 'loan')
    .eq('target_role', 'admin') // Admin melihat request milik Anggota
    .eq('status', 'pending')
    .order('created_at', { ascending: true })
    .limit(1); // Sesuai instruksi: Tampilkan 1 per 1 bertahap
  
  const activeLoanRequest = loanRequests?.[0] || null;
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* --- BANNER REQUEST ANGGOTA BARU --- */}
        {pendingMembers.length > 0 && (
          <section className="bg-blue-50 border border-blue-200 rounded-3xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-blue-200 flex justify-between items-center bg-blue-600">
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <span>🔔</span> Ada {pendingMembers.length} Permintaan Bergabung!
              </h2>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <tbody>
                  {pendingMembers.map((member) => (
                    <tr key={member.id} className="border-b border-blue-100 last:border-0 text-sm">
                      <td className="p-4">
                      ...
                      <td className="p-4 text-right">
                        <MemberRequestActions userId={member.id} />
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <LoanBanner request={activeLoanRequest} />
        ...
        <header className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Transparansi Kas</h1>
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Link href="/admin/tambah" className="bg-white hover:bg-emerald-50 border border-gray-100 p-4 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-2 transition group">
            ...
            <span className="font-semibold text-gray-700 text-sm text-center">Catat Kas</span>
          </Link>
          <Link href="/admin/target" className="bg-white hover:bg-blue-50 border border-gray-100 p-4 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-2 transition group">
            ...
            <span className="font-semibold text-gray-700 text-sm text-center">Set Target</span>
          </Link>
          ...
          <Link href="/admin/pinjam" className="bg-white hover:bg-rose-50 border border-gray-100 p-4 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-2 transition group">
            ...
            <span className="font-semibold text-gray-700 text-sm text-center">Pinjaman</span>
          </Link>
        </section>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-6 text-center text-gray-500 italic">Belum ada data kas.</td>
                  </tr>
                ) : (
                  transactions.map((trx) => (
                    <tr key={trx.id} className="hover:bg-gray-50/50 text-gray-700 text-sm">
                      ...
                      <td className="p-4 border-b">
                        ...
                        <p className="text-xs text-gray-500">Oleh: {(trx.users as any)?.full_name || 'User Terhapus'}</p>
