...
import QrisViewer from './components/QrisViewer';
...
export default async function Home() {
  ...
  // 3. Fetch Data QRIS dari class_settings
  ...
  // 4. Kalkulasi Saldo (Hanya yang completed)
  ...
  if (currentTarget && currentTarget.target_amount > 0) {
    ...
    // Cegah persentase tembus lebih dari 100% atau kurang dari 0%
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-3xl mx-auto space-y-6"> {/* max-w diperkecil dikit biar formasi 2x2 nya cakep */}
        ...
        <header className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Uang Kas Kelas</h1>
              <p className="text-gray-500 text-sm mt-1">Total Saldo Aktif Saat Ini</p>
            </div>
              ...
              <div className="flex justify-between items-end mb-3">
                <div>
                  <p className="text-sm text-blue-800 font-bold">
                    Target {currentTarget.period_type.charAt(0).toUpperCase() + currentTarget.period_type.slice(1)}
                  </p>
                  <p className="text-xs text-blue-600/80 mt-0.5">{currentTarget.description || 'Tidak ada keterangan'}</p>
                </div>
        ...
        <section className="grid grid-cols-2 gap-4">
          <Link href="/tambah" className="bg-white hover:bg-blue-50 border border-gray-100 p-5 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-3 transition group">
            <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">📝</div>
            <span className="font-semibold text-gray-700 text-sm">Catat Kas</span>
          </Link>

          {user.role === 'admin' ? (
            <Link href="/target" className="bg-white hover:bg-purple-50 border border-gray-100 p-5 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-3 transition group">
              ...
              <span className="font-semibold text-gray-700 text-sm">Atur Target</span>
            </Link>
          ) : (
            <div className="bg-gray-50 border border-gray-100 p-5 rounded-3xl flex flex-col items-center justify-center gap-3 cursor-not-allowed opacity-60">
              <div className="w-12 h-12 rounded-full bg-gray-200 text-gray-500 flex items-center justify-center text-xl">🔒</div>
              <span className="font-semibold text-gray-500 text-sm">Atur Target</span>
            </div>
          )}
          ...
          <Link href="/report" className="bg-white hover:bg-amber-50 border border-gray-100 p-5 rounded-3xl shadow-sm flex flex-col items-center justify-center gap-3 transition group">
            ...
            <span className="font-semibold text-gray-700 text-sm">Laporan</span>
          </Link>
        </section>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-600 text-sm">
                  ...
                  <th className="p-4 font-medium border-b">Tipe</th>
                  <th className="p-4 font-medium border-b">Status</th>
                  <th className="p-4 font-medium border-b text-right">Nominal</th>
                  {user.role === 'admin' && <th className="p-4 font-medium border-b text-right">Aksi</th>}
                </tr>
              </thead>
              <tbody>
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan={user.role === 'admin' ? 6 : 5} className="p-6 text-center text-gray-500 italic">
                      Belum ada data kas yang tercatat.
                    </td>
                  </tr>
                ) : (
                  transactions.map((trx) => (
                    <tr key={trx.id} className="hover:bg-gray-50/50 text-gray-700 text-sm">
                      ...
                      <td className="p-4 border-b">
                        ...
                        {trx.proof_image_url && (
                           <a href={trx.proof_image_url} target="_blank" className="text-xs text-blue-500 hover:text-blue-700 hover:underline mt-1 block font-medium">Lihat Bukti</a>
                        )}
                      </td>
                      <td className="p-4 border-b">
                      ...
                      <td className={`p-4 border-b text-right font-bold whitespace-nowrap ${
